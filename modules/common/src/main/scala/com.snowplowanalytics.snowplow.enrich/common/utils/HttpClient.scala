...
object HttpClient {
  ...
  private[utils] def getHeaders(authUser: Option[String], authPassword: Option[String]): Headers = {
    val alwaysIncludedHeaders = List(Header.Raw(CIString("content-type"), "application/json"), Header.Raw(CIString("accept"), "*/*"))
    if (authUser.isDefined || authPassword.isDefined)
      Headers(Authorization(BasicCredentials(authUser.getOrElse(""), authPassword.getOrElse("")))) ++ Headers(alwaysIncludedHeaders)
            ...
            http4sClient
              ...
              .use { response =>
                ...
                response.status.responseClass match {
                  ...
                  case _ => Sync[F].pure(new Throwable(s"Request failed with status ${response.status.code}").asLeft[String])
                }
              }
