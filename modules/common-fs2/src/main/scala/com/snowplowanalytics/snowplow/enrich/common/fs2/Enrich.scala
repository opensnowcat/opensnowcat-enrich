...
import cats.effect.kernel.{Clock, Fiber, Outcome}
...
object Enrich {
  ...
  def run[F[_]: Async: Temporal, A](env: Environment[F, A]): Stream[F, Unit] = {
    ...
    val enriched =
      env.source.chunks
        ...
        .evalMap(chunk =>
          for {
            begin <- Clock[F].realTime.map(_.toMillis)
            result <-
   ...
   */
  def enrichWith[F[_]: RegistryLookup: Sync](
  ...
  def sendToSentry[F[_]: Sync](
    ...
    for {
      ...
      now <- Sync[F].realTime.map(fd => Instant.ofEpochMilli(fd.toMillis))
  ...
  def sinkChunk[F[_]: Sync: Parallel, A](
    chunk: List[Result],
    env: Environment[F, A]
  ): F[Unit] = {
    val (bad, enriched) =
   ...
   */
  private def runWithShutdown[F[_]: Concurrent: Async: Temporal, A](
    enriched: Stream[F, List[(A, Result)]],
    sinkAndCheckpoint: Pipe[F, List[(A, Result)], Unit]
  ): F[Unit] =
    Queue.bounded[F, Option[List[(A, Result)]]](1).flatMap { queue =>
      Stream
  ...
  private def terminateStream[F[_]: Concurrent: Temporal, A](
    ...
    for {
      timeout <- Temporal[F].pure(5.minutes)
      _ <- logger.warn(s"Terminating enrich stream. Waiting $timeout for it to complete")
      _ <- queue.offer(None)
      _ <- fiber.join.timeoutTo(
        timeout,
        logger.warn(s"Stream not complete after $timeout, canceling") >>
          fiber.cancel.as(Outcome.Canceled[F, Throwable, Unit](): Outcome[F, Throwable, Unit])
      )
    } yield ()
}
